...
function getOrCreateSheet() {
  ...
  if (!sheet) {
    ...
    const headers = [
      // 1. Admin Basics
      "Caregiver ID", "First Name", "Last Name", "Phone", "Email", "Title", "Status", "Created At", "App Status",
      
      // 2. Personal & Legal
      "Middle Int", "DOB", "Gender", "SSN", "Address", "Apt", "City", "State", "Zip", "US Eligible?", "US Citizen?",
      
      // 3. Transportation
      "Driver License?", "License State", "License #", "Has Car?", "Car Make/Model", "Has Insurance?",
      
      // 4. Position & Availability
      "Hours Avail", "Schedule Desired", "Times Avail", "Emergency Avail?", "Live-in Avail?",
      
      // 5. Certs & Skills
      "Certifications", "Skills Checklist", "Languages",
      
      // 6. Education
      "High School", "HS Degree", "College", "College Degree", "Other Edu", "Other Degree",
      
      // 7. Employment History (JSON String for simplicity or summary)
      "Employer 1", "Employer 2", "Employer 3",
      
      // 8. References & Emergency
      "Reference 1", "Reference 2", "Emergency Contact", "Emerg. Phone", "Emerg. Relation",
      
      // 9. Legal & Signature
      "Criminal Conviction?", "Conviction Details", "Signature Name", "Signature Date", "Agreed to Terms"
    ];
    
    const range = sheet.getRange(1, 1, 1, headers.length);
    range.setValues([headers]);
    range.setBackground(PRIMARY_COLOR).setFontColor("white").setFontWeight("bold").setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP);
...
function submitFullApplication(form) {
  try {
    ...
    const dataToUpdate = [[
      ...
      `${form.hsName} - ${form.hsCity}`, form.hsDegree,
      `${form.colName} - ${form.colCity}`, form.colDegree,
      form.otherEdu, form.otherDegree,
      ...
      refStr(form.ref1),
      refStr(form.ref2),
      form.emName, form.emPhone, form.emRel,
...
function getDashboardStats() {
  const sheet = getOrCreateSheet();
  if (sheet.getLastRow() <= 1) return { total: 0, completed: 0, active: 0, inactive: 0, stna: 0 };
  
  const data = sheet.getRange(2, 6, sheet.getLastRow()-1, 4).getValues(); // Get Title(6), Status(7), ... App Status(9)
  
  let stats = { total: data.length, completed: 0, active: 0, inactive: 0, stna: 0 };
